...
species player skills: [moving] control: simple_bdi {
    ...
    // PERCEPTIONS
    perceive target: breezeArea in: 2 {
        // Only ignore danger signals occasionally (20% chance instead of 30-70%)
        if (flip(0.8)) {  // 80% detection rate instead of based on risk_tolerance
    ...
    // Take risks after being stuck for a while
    if (stuck_counter > 50 or (stuck_counter > 30 and flip(risk_tolerance))) {
        write "Stuck too long - taking calculated risk!";
        stuck_counter <- 0;
        
        list<gworld> neighbors <- my_cell.neighbors;
        gworld best_neighbor <- nil;
        float min_danger_score <- 9999.0;
        
        // Calculate danger level for each neighbor by checking ALL hazards
        loop neighbor over: neighbors {
            float danger_score <- 0.0;
            
            // Check distance to ALL pits (no at_distance filter)
            ask pitArea {
                float dist <- self.location distance_to neighbor.location;
                
                // Exact location = certain death (high score)
                if (dist = 0) {
                    danger_score <- danger_score + 1000.0;
                }
                // Very close = high danger
                else if (dist < 2.0) {
                    danger_score <- danger_score + 50.0;
                }
                // Close = moderate danger
                else if (dist < 5.0) {
                    danger_score <- danger_score + 10.0 / dist;
                }
                // Far = low danger (inversely proportional)
                else {
                    danger_score <- danger_score + 2.0 / dist;
                }
            }
            
            // Check distance to ALL wumpus (no at_distance filter)
            ask wumpusArea {
                float dist <- self.location distance_to neighbor.location;
                
                // Wumpus is more dangerous than pits
                if (dist = 0) {
                    danger_score <- danger_score + 1500.0;
                }
                else if (dist < 2.0) {
                    danger_score <- danger_score + 75.0;
                }
                else if (dist < 5.0) {
                    danger_score <- danger_score + 15.0 / dist;
                }
                else {
                    danger_score <- danger_score + 3.0 / dist;
                }
            }
            
            // Track the neighbor with minimum danger score
        ...
        if (best_neighbor != nil) {
            ...
            previous_cell <- my_cell;
            my_cell <- best_neighbor;
            location <- my_cell.location;
        }
    } 
    else {
        // Normal safe retreat behavior (backtrack to previous safe position)
    ...
    // Clear all danger beliefs and intentions
    ...
    plan patrol_plan intention: patrol_desire priority: 1 {
        list<gworld> all_neighbors <- my_cell.neighbors;
        list<gworld> safe_neighbors <- [];
        list<gworld> moderately_risky <- [];
        list<gworld> very_risky <- [];
        
        loop neighbor over: all_neighbors {
            bool is_pit <- false;
            ...
            ask pitArea {
			    if (self.location = neighbor.location) {
			        is_pit <- true;
			    } else if (self.location distance_to neighbor.location < 2.0) { 
			        near_danger <- true;
			    }
			}
            ...
            ask wumpusArea {
                if (self.location = neighbor.location) {
                    is_wumpus <- true;
                } else if (self.location distance_to neighbor.location < 8) {
                    near_danger <- true;
                }
            }
            ...
            if (is_pit or is_wumpus) {
                add neighbor to: very_risky;
            } else if (near_danger) {
                add neighbor to: moderately_risky;
            } else {
                add neighbor to: safe_neighbors;
            }
        }
        
        // Decision logic with multiple tiers
        if (!empty(safe_neighbors)) {
            safe_moves_count <- safe_moves_count + 1;
            
            // If been safe for too long (30 moves), take moderate risk for exploration
            if (safe_moves_count > 30 and !empty(moderately_risky) and flip(curiosity)) {
                write "Safe for too long - exploring moderate risk area";
                safe_moves_count <- 0;
                previous_cell <- my_cell;
                my_cell <- one_of(moderately_risky);
                location <- my_cell.location;
            } else {
        ...
        } else if (!empty(moderately_risky)) {
            // No safe moves - choose moderate risk
            write "No safe options - taking moderate risk";
            safe_moves_count <- 0;
            previous_cell <- my_cell;
            my_cell <- one_of(moderately_risky);
            location <- my_cell.location;
        } else if (!empty(very_risky) and flip(0.1)) {
            // Last resort - very low probability of extreme risk
            write "Desperate move - high risk!";
            safe_moves_count <- 0;
            previous_cell <- my_cell;
            my_cell <- one_of(very_risky);
            location <- my_cell.location;
        }
        // Otherwise stay put
    }
    ...
    ask wumpusArea {
        if (self.location = myself.location) {
            write "Player eaten by Wumpus at step " + steps_taken + " at location " + myself.location;
            ask myself {
    ...
    reflex save_all_results {
        ask simulations {
            ...
            save [
                ...
                self.grid_size
            ] to: "full_exploration_results_risk.csv" 
            rewrite: false
            format: "csv";
        }
